		...
		if line, err := reader.ReadString('\n'); err != nil {
			...
			case "login":
				if subParts := strings.SplitN(parts[1], " ", 3); len(subParts) != 3 {
					...
					if err := attendant.Send("my-auth.login", Args{subParts[1], subParts[2], "main"}, nil); err != nil {
			...
			case "shout":
				if subParts := strings.SplitN(parts[1], " ", 2); len(subParts) != 2 {
					...
					if err := attendant.Send("chat.MSG", Args{subParts[1]}, nil); err != nil {
			...
			case "whisper":
				if subParts := strings.SplitN(parts[1], " ", 3); len(subParts) != 3 {
					...
					if err := attendant.Send("chat.PMSG", Args{subParts[1], subParts[2]}, nil); err != nil {
			...
			case "logout":
				if subParts := strings.SplitN(parts[1], " ", 1); len(subParts) != 1 {
					...
					if err := attendant.Send("my-auth.logout", nil, nil); err != nil {
