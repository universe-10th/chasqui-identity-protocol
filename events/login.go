...
type LoginCallback func(identifier interface{}, password string, credential credentials.Credential, err error)
...
// any panic.
func (event *LoginEvent) trigger(callback LoginCallback, identifier interface{}, password string, credential credentials.Credential, err error) {
	defer func() { recover() }()
	callback(identifier, password, credential, err)
}

// Triggers all the callbacks. Hopefully, few callbacks will be triggered.
func (event *LoginEvent) Trigger(identifier interface{}, password string, credential credentials.Credential, err error) {
	for _, callback := range event.callbacks {
		event.trigger(callback, identifier, password, credential, err)
	}
}
